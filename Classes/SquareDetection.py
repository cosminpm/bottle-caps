...
class SquareDetection:
    def __init__(self, points: list[tuple[int]], pMaxX: tuple[int, int], pMinX: tuple[int, int], pMaxY: tuple[int, int],
                 pMinY: tuple[int, int], name, img=None):
        self.points = points
    ...
    def calc_centroid(self) -> tuple[int, ...]:
        cen = np.array(self.points).mean(axis=0)
        tp = tuple([int(i) for i in cen])
        return tp

    def calc_distance(self, pMaxX: tuple[int, int], pMinX: tuple[int, int], pMaxY: tuple[int, int],
                      pMinY: tuple[int, int]) -> float:
    ...
    def draw_centroid(self) -> None:
        self.img = cv2.circle(self.img, (self.centroid[0], self.centroid[1]), radius=0, color=(125, 0, 255),
                              thickness=5)

    def get_bot_and_top_corners(self) -> tuple[tuple[int, int], tuple[int, int]]:
    ...
    def draw_percentage(self) -> np.ndarray:
        crop = self.get_cropped_img()
    ...
    def draw_name(self) -> np.ndarray:
        crop = self.get_cropped_img()
        photo_img = cv2.putText(self.img, self.name, crop[1],
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLOR_PERCENTAGE, 1, cv2.LINE_AA)
        return photo_img

    def is_overlap(self, aux_square: SquareDetection) -> int:
